...
CENARIO                    = "03"
RAIO                       = 480.0
CAM_ALTURA_OFFSET          = 150.0  # Altura extra da câmara (em cm) para evitar clipping com o chão/terreno
NUM_ANGULOS                = 60
BBOX_MAX_FRAC              = 0.80
BBOX_MIN_FRAC              = 0.03
MARGEM_BORDA               = 20
...
def focar_piplup(piplup_alvo, todos_pips):
    target_name = piplup_alvo.get_name() if piplup_alvo else ""
    for pip in todos_pips:
        pip.set_is_temporarily_hidden_in_editor(pip.get_name() != target_name)
...
def ator_pertence_ao_piplup(hit_actor, piplup_actor):
    if not hit_actor or not piplup_actor:
        return False

    try:
        hit_name = hit_actor.get_name()
        target_name = piplup_actor.get_name()
    ...
    try:
        ...
        if owner and owner.get_name() == target_name:
            return True
    except Exception:
        pass
...
def extrair_hit_info(hit):
    if isinstance(hit, tuple):
        ...
        if hit_result:
            try:
                hit_success = hasattr(hit_result, 'blocking_hit') and hit_result.blocking_hit
            except Exception:
                hit_success = False

    hit_actor = None
    if hit_success and hit_result:
        try:
...
def calcular_posicoes():
    ...
    for piplup in pips:
        ...
        for j in range(NUM_ANGULOS):
            ...
            # Adicionado o CAM_ALTURA_OFFSET para a câmara orbitar ligeiramente acima do ator
...
def on_tick(dt):
    ...
    if fase == "mover":
        if i == 0 or posicoes[i]["piplup"].get_name() != posicoes[i - 1]["piplup"].get_name():
...
if total > 0:
    ...
    unreal.log(f"  Raio: {RAIO}cm | Altura Offset: {CAM_ALTURA_OFFSET}cm | Margem borda: {MARGEM_BORDA}px")
    log_paths()
