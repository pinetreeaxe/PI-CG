...
# Paths dos materiais (confirmar com Copy Reference no Content Browser)
...
def aplicar_material(piplup_actor, mat):
    ...
    unreal.log(f"[MAT] {piplup_actor.get_actor_label()} — {len(comps)} componente(s)")
    for comp in comps:
    ...
    if not comps:
        try:
            ...
            for slot in range(comp.get_num_materials()):
                comp.set_material(slot, mat)
            unreal.log(f"[MAT] fallback static_mesh_component OK")
...
def on_tick(dt):
    ...
    # ── FASE 1: mover câmara + aplicar emissivo ──────────────────
    if fase == "mover":
    ...
    # ── FASE 4: detetar bbox no screenshot emissivo ──────────────
        ...
        if estado["espera"] >= FRAMES_FICHEIRO:
            ...
            # Screenshot emissivo é sempre apagado — não é para treino
            ...
            # Bbox válida — voltar ao material normal para a foto final
    ...
    # ── FASE 5: esperar render com material normal ────────────────
    elif fase == "esperar_normal":
        estado["espera"] += 1
        if estado["espera"] >= FRAMES_ESPERA:
            estado["fase"] = "capturar_normal"

    # ── FASE 6: capturar screenshot final (para treino) ──────────
    ...
    # ── FASE 7: guardar imagem de treino + label YOLO ─────────────
